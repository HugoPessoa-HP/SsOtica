...
// src/services/CentroGuarapari_02/GetVendasService.ts
var GetVendasService_exports = {};
__export(GetVendasService_exports, {
  GetVendasMuquicaba_Service: () => GetVendasMuquicaba_Service
});
module.exports = __toCommonJS(GetVendasService_exports);
  ...
  execute() {
    return __async(this, null, function* () {
      ...
      for (i = 0; i < lengthData; i++) {
        var nome = yield data[i].cliente.nome;
        var numero = yield data[i].cliente.telefones;
        var values = yield Object.values(numero[0]);
        var numeroMudado = JSON.stringify(values);
        numeroMudado = yield numeroMudado.replace(/\D/g, "");
        var email = yield data[i].valor_liquido;
        yield console.log(numeroMudado);
      }
